...
```mermaid
flowchart TD
    A[Bundesland und Gemeinde festlegen] --> B{Private Zweitnutzung<br>und touristische Nutzung zulässig?}
    B -- Nein --> X[Objekt oder Standort verwerfen]
    B -- Ja --> C{Firmen-/Tagungsnutzung baurechtlich<br>und gewerberechtlich tragfähig?}
    C -- Nein --> D[Modell auf private Nutzung plus begrenzte Vermietung beschränken]
    C -- Ja --> E{Privatnutzung dominiert?}
    E -- Ja --> F[Miteigentum oder schlanke Personengesellschaft prüfen]
    E -- Nein --> G[KG, GmbH oder GmbH & Co KG prüfen]
    F --> H{Bank finanziert ohne ungewollte Vollhaftungen?}
    G --> H
...
```mermaid
timeline
    title Nächste Schritte für das Wiki und die Projektvorbereitung
    Woche eins : Bundesland und Zielgemeinden festlegen
               : Widmungs- und Nutzungsfragen als Go/No-Go definieren
    Woche zwei : Standard-Dokumentliste für Verkäufer und Gemeinde erstellen
               : steuerliche Grundsatzentscheidung zu USt-Szenarien dokumentieren
